...
test('live catalog data can be opened and added to the local cart', async ({ page }) => {
  ...
  const addToGear = page.getByRole('button', { name: 'ADD TO GEAR', exact: true });
  await expect(addToGear).toBeVisible({ timeout: 20_000 });
  await expect(page.getByText('12 Customer Reviews', { exact: true })).toHaveCount(0);
  await expect(addToGear).toHaveCSS('background-color', 'rgb(255, 102, 0)');
  await expect(addToGear).toHaveCSS('color', 'rgb(7, 7, 7)');
  await addToGear.hover();
  await expect(addToGear).toHaveCSS('background-color', 'rgb(224, 88, 0)');
  await expect(addToGear).toHaveCSS('color', 'rgb(7, 7, 7)');
  const productDescription = page.getByTestId('product-description');
  await expect(productDescription).toBeVisible();
  await expect(productDescription).not.toHaveText('');
  await addToGear.click();
