...
export default function ReviewRatingDistribution() {
  ...
  return (
    <div className="space-y-3 bg-[#121212] border border-[#2A2A2A] p-4 clip-angled my-3">
        ...
        {ratings.map((r) => (
          <div key={r.stars} className="flex items-center gap-3 text-xs font-mono">
            <span className="w-12 text-neutral-400 flex items-center gap-1">
              {r.stars} <Star className="w-3 h-3 text-[#FF6600] fill-current" />
            </span>
            ...
            <div className="flex-1 h-2 bg-[#1E1E1E] rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-gradient-to-r from-[#2F4F2F] to-[#FF6600]"
              />
            </div>
